...
type Route = "overview" | "credentials" | "sessions" | "catalog" | "usage" | "charges" | "profile"
type AuthState =
...
const routes: readonly Route[] = ["overview", "credentials", "sessions", "catalog", "usage", "charges", "profile"]
const routeFromLocation = (): Route => {
  const candidate = window.location.pathname.replace(/^\/+|\/+$/gu, "") || "overview"
  return routes.includes(candidate as Route) ? candidate as Route : "overview"
}
...
const renderCredential = (item: Credential): TemplateResult => html`
  <tr><th scope="row">${item.label}</th><td><code>${item.prefix}…</code></td><td>${item.status}</td>
...
const renderSession = (item: SignerSessionMetadata): TemplateResult => html`
  <article part="card"><h3><code>${item.id}</code></h3><dl>
  ...
  </dl><p><button type="button" data-action="refresh-session" data-id=${item.id.toString()}>Refresh</button>
...
const renderCatalogEntry = (item: CatalogEntry): TemplateResult => html`
  ...
  <td>${item.available ? "Available" : "Unavailable"}</td></tr>`
...
export class UserApp extends LitElement {
  static styles = css`
    :host { display: block; }
    section, form, fieldset, article, table { margin-block: var(--och-space-3); }
    form, fieldset, article, dl { display: grid; gap: var(--och-space-2); }
    table { inline-size: 100%; border-collapse: collapse; }
    th, td { padding: var(--och-space-1); text-align: start; border-block-end: var(--och-border-width, 1px) solid var(--och-color-border); }
    dialog { max-inline-size: min(var(--och-content-max), calc(100vi - 2 * var(--och-space-3))); color: inherit; background: var(--och-color-surface-raised); border: var(--och-border-width, 1px) solid var(--och-color-border); border-radius: var(--och-radius); }
    code { overflow-wrap: anywhere; }
  `
  @state() private auth: AuthState = { tag: "checking" }
  @state() private providers: readonly Provider[] = []
  @state() private route: Route = routeFromLocation()
  ...
  private renderNotice(): TemplateResult | typeof nothing {
    ...
    return this.notice.tone === "error" ? html`<p role="alert">${this.notice.text}</p>` : html`<p role="status">${this.notice.text}</p>`
  }
  private renderSignIn(): TemplateResult {
    return html`<section aria-labelledby="sign-in-heading"><h2 id="sign-in-heading">${msg("Sign in")}</h2>
      <form @submit=${this.requestCode}><label for="request-email">Email address</label>
        <input id="request-email" name="email" type="email" autocomplete="email" maxlength="320" required>
        <button type="submit" ?disabled=${this.inFlight.has("request-code")}>Send one-time code</button></form>
      <form @submit=${this.verifyCode}><fieldset><legend>Verify your one-time code</legend>
        ...
        </fieldset><button type="submit" ?disabled=${this.inFlight.has("verify-code")}>Verify and sign in</button></form>
      ${this.providers.some((provider) => provider !== "email") ? html`<h3>Other sign-in options</h3><ul>${this.providers.map(renderProvider)}</ul>` : nothing}
    </section>`
  }
  private renderUnscoped(session: AuthSession): TemplateResult {
    return html`<section aria-labelledby="link-heading"><h2 id="link-heading">Connect your account</h2>
      <p>Your verified identity is not connected to a clearinghouse account yet.</p><p>Principal ID: <code>${session.principal_id}</code></p>
      <form @submit=${this.redeem}><label for="invitation">One-time invitation</label>
        <input id="invitation" name="invitation" type="password" autocomplete="off" minlength="50" maxlength="128" required>
        <button type="submit" ?disabled=${this.inFlight.has("redeem")}>Connect identity</button></form></section>`
  }
  private renderOverview(): TemplateResult {
    return html`<section aria-labelledby="overview-heading"><h2 id="overview-heading">${msg("Account overview")}</h2>
      ${this.account === undefined || this.balance === undefined ? html`<p role="status">Loading account…</p>` : html`
        <h3>${this.account.display_name}</h3><dl><div><dt>Status</dt><dd>${this.account.status}</dd></div>
        <div><dt>Posted balance</dt><dd>${this.balance.posted.amount} ${this.balance.posted.unit}</dd></div>
        <div><dt>Open exposure</dt><dd>${this.balance.open_lease_exposure.amount} ${this.balance.open_lease_exposure.unit}</dd></div>
        <div><dt>Available</dt><dd>${this.balance.available.amount} ${this.balance.available.unit}</dd></div></dl>`}</section>`
  }
  private renderCredentials(): TemplateResult {
    return html`<section aria-labelledby="credentials-heading"><h2 id="credentials-heading">Credentials</h2>
      <form @submit=${this.issueCredential}><label for="credential-label">Credential label</label>
        <input id="credential-label" name="label" maxlength="200" required><button type="submit" ?disabled=${this.inFlight.has("issue-credential")}>Create credential</button></form>
      <table><caption>Your API credentials</caption><thead><tr><th scope="col">Label</th><th scope="col">Prefix</th>
      <th scope="col">Status</th><th scope="col">Created</th><th scope="col">Actions</th></tr></thead>
      <tbody>${this.credentials.map(renderCredential)}</tbody></table></section>`
  }
  private renderSessions(): TemplateResult {
    return html`<section aria-labelledby="sessions-heading"><h2 id="sessions-heading">Signer sessions</h2>
      <form @submit=${this.createSession}><fieldset><legend>Open a bounded signer session</legend>
        ...
        </fieldset><button type="submit" ?disabled=${this.inFlight.has("create-session")}>Create signer session</button></form>
      ${this.sessions.length === 0 ? html`<p>No signer sessions.</p>` : this.sessions.map(renderSession)}</section>`
  }
  private renderCatalog(): TemplateResult {
    return html`<section aria-labelledby="catalog-heading"><h2 id="catalog-heading">Capability catalog</h2>
      <table><caption>Capabilities and exact rates available to your account</caption><thead><tr><th scope="col">Capability</th>
      <th scope="col">Model</th><th scope="col">Rate</th><th scope="col">Policy</th></tr></thead>
      <tbody>${this.catalog.map(renderCatalogEntry)}</tbody></table></section>`
  }
  private renderUsage(): TemplateResult {
    return html`<section aria-labelledby="usage-heading"><h2 id="usage-heading">Usage</h2><table><caption>Settled usage for your account</caption>
      <thead><tr><th scope="col">Occurred</th><th scope="col">Capability</th><th scope="col">Quantity</th><th scope="col">Event</th></tr></thead>
      <tbody>${this.usage.map(renderUsage)}</tbody></table>
      ${typeof this.usageCursor === "string" ? html`<p><button type="button" data-action="load-more-usage"
        ?disabled=${this.inFlight.has("load-more-usage")}>Load more usage</button></p>` : nothing}</section>`
  }
  private renderCharges(): TemplateResult {
    return html`<section aria-labelledby="charges-heading"><h2 id="charges-heading">Charges</h2><table><caption>Settled charges for your account</caption>
      <thead><tr><th scope="col">Created</th><th scope="col">Amount</th><th scope="col">Usage event</th></tr></thead>
      <tbody>${this.charges.map(renderCharge)}</tbody></table>
      ${typeof this.chargesCursor === "string" ? html`<p><button type="button" data-action="load-more-charges"
        ?disabled=${this.inFlight.has("load-more-charges")}>Load more charges</button></p>` : nothing}</section>`
  }
  private renderProfile(session: AuthSession): TemplateResult {
    return html`<section aria-labelledby="profile-heading"><h2 id="profile-heading">Profile and security</h2><dl>
      ...
      <p><button type="button" data-action="refresh-browser" ?disabled=${this.inFlight.has("refresh-browser")}>Refresh browser session</button>
      <button type="button" data-action="logout" ?disabled=${this.inFlight.has("logout")}>Sign out</button></p></section>`
  }
  ...
  private renderNavigation(): TemplateResult {
    const link = (route: Route, label: string) => html`<li><a href=${route === "overview" ? "/" : `/${route}`}
      aria-current=${this.route === route ? "page" : "false"} @click=${this.navigate}>${label}</a></li>`
    return html`<ul slot="navigation" part="navigation-list">${link("overview", "Overview")}${link("credentials", "Credentials")}
      ${link("sessions", "Sessions")}${link("catalog", "Catalog")}${link("usage", "Usage")}${link("charges", "Charges")}${link("profile", "Profile")}</ul>`
  }
  protected render(): TemplateResult {
    return html`<och-app-shell @click=${this.handleAction} heading="Clearinghouse account" summary="Understand and control walletless Livepeer network spend."
      exportparts="skip-link, header, brand, navigation, main, heading-group, title, summary, footer, footer-note">
      ${this.auth.tag === "scoped" ? this.renderNavigation() : nothing}${this.renderNotice()}
      ${this.auth.tag === "checking" ? html`<p role="status">Checking your session…</p>` : nothing}
      ${this.auth.tag === "denied" ? html`<section aria-labelledby="denied-heading"><h2 id="denied-heading">Access denied</h2><p>This identity cannot use the account application.</p></section>` : nothing}
      ${this.auth.tag === "unavailable" ? html`<section aria-labelledby="unavailable-heading"><h2 id="unavailable-heading">Service unavailable</h2><p>Your session could not be checked safely. Try again later.</p></section>` : nothing}
      ...
      ${this.secret === undefined ? nothing : html`<dialog @close=${this.clearSecret} aria-labelledby="secret-heading">
