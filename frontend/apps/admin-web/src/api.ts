...
export type Failure = ApiFailure | InvalidPayload | NetworkFailure
const csrf = (): string => document.cookie.split("; ").find((value) => value.startsWith("och_csrf="))?.slice(9) ?? ""
const response = (operation: string, path: string, init?: RequestInit): Effect.Effect<Response, Failure> => Effect.tryPromise({
  try: (signal) => fetch(path, { credentials: "same-origin", ...init, signal }),
  catch: (cause) => new NetworkFailure({ operation, cause })
}).pipe(
  Effect.timeout("10 seconds"),
  Effect.mapError((cause) => cause instanceof NetworkFailure ? cause : new NetworkFailure({ operation, cause })),
  Effect.flatMap((value) => value.ok ? Effect.succeed(value) : Effect.fail(new ApiFailure({ status: value.status, operation })))
)
const decode = <A, I>(operation: string, schema: Schema.Schema<A, I, never>, path: string, init?: RequestInit): Effect.Effect<A, Failure> => response(operation, path, init).pipe(
  Effect.flatMap((value) => Effect.tryPromise({ try: () => value.json(), catch: (cause) => new InvalidPayload({ operation, cause }) })),
...
export const AdminApiLive = Layer.succeed(AdminApi, {
  ...
  session: () => decode("check administrator session", Session, "/v1/auth/session"),
  requestCode: (email) => response("send a one-time code", "/v1/auth/email/code", mutation("POST", { email })).pipe(Effect.asVoid),
  verifyCode: (email, code) => decode("verify the one-time code", Session, "/v1/auth/email/verify", mutation("POST", { email, code })),
  logout: () => response("sign out", "/v1/auth/session", mutation("DELETE")).pipe(Effect.asVoid),
  overview: () => decode("load the administration overview", Overview, "/v1/admin/overview"),
  users: (cursor) => decode("load users and accounts", Users, collectionPath("/v1/admin/users", cursor)),
  workloads: (cursor) => decode("load workloads", Workloads, collectionPath("/v1/admin/workloads", cursor)),
  setStop: (enabled, reason) => decode(enabled ? "pause signer authorization" : "resume signer authorization", Stop, "/v1/admin/global-stop", mutation("PUT", { enabled, reason }))
})
